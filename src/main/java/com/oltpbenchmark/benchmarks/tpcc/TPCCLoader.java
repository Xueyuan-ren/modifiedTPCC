...
public class TPCCLoader extends Loader<TPCCBenchmark> {
    ...
    private final int itemFactor;
    private final double skewFactor;
    private static ZipfDistribution zipf = null;
    // This map holds the number of items per warehouse
    private static final Map<Integer, Integer> numItemsPerWarehouse = new ConcurrentHashMap<>();
    // This map holds the item IDs per warehouse
    private static final Map<Integer, List<Integer>> itemIdsPerWarehouse = new ConcurrentHashMap<>();
    // Store generated o_id for each district
    private static final Map<String, List<Integer>> oidsPerDistrict = new ConcurrentHashMap<>();


    public TPCCLoader(TPCCBenchmark benchmark) {
        super(benchmark);
        numWarehouses = Math.max(Math.round(TPCCConfig.configWhseCount * this.scaleFactor), 1);
        itemFactor = this.workConf.getItemFactor();
        skewFactor = this.workConf.getSkewFactor();
        // TPC-C default is 100,000 items (itemFactor = 1)
        // We scale this by the itemFactor to allow for larger item counts
        itemCount = TPCCConfig.configItemCount * itemFactor;
        if (skewFactor > 0.0) {
            zipf = new ZipfDistribution(itemCount, skewFactor);
        }
    }

    public static ZipfDistribution getZipfDistribution() {
        return zipf;
    }

    public static Map<Integer, Integer> getNumItemsPerWarehouse() {
        return numItemsPerWarehouse;
    }

    public static Map<Integer, List<Integer>> getItemIdsPerWarehouse() {
        return itemIdsPerWarehouse;
    }
        ...
        for (int w = 1; w <= numWarehouses; w++) {
            ...
            LoaderThread t = new LoaderThread(this.benchmark) {
                ...
                public void load(Connection conn) {
                    ...
                    // STOCK
                    if (zipf != null) {
                        loadSkewedStock(conn, w_id, itemCount);
                    } else {
                        // If skewFactor is 0, we use the original uniform distribution
                        loadStock(conn, w_id, itemCount);
                    }
    ...
    protected void loadSkewedStock(Connection conn, int w_id, int numItems) {

        int k = 0;

        try (PreparedStatement stockPreparedStatement = getInsertStatement(conn, TPCCConstants.TABLENAME_STOCK)) {

            int numItemsForWarehouse = zipf.sample();
            numItemsPerWarehouse.put(w_id, numItemsForWarehouse);

            List<Integer> itemsToLoad = new ArrayList<>();
            Random rnd = new Random(w_id); // Seed with warehouse ID for reproducibility
            Set<Integer> uniqueItems = new HashSet<>(numItemsForWarehouse);
            while (uniqueItems.size() < numItemsForWarehouse) {
                int itemId = rnd.nextInt(numItems) + 1;
                uniqueItems.add(itemId);
            }
            itemsToLoad.addAll(uniqueItems);
            itemIdsPerWarehouse.put(w_id, itemsToLoad);

            for (int i_id : itemsToLoad) {
                Stock stock = new Stock();
                stock.s_i_id = i_id;
                stock.s_w_id = w_id;
                stock.s_quantity = TPCCUtil.randomNumber(10, 100, benchmark.rng());
    ...
    protected void loadOpenOrders(Connection conn, int w_id, int districtsPerWarehouse, int customersPerDistrict) {
        ...
        try (PreparedStatement openOrderStatement = conn.prepareStatement(
                "INSERT INTO oorder (o_w_id, o_d_id, o_c_id, o_carrier_id, o_ol_cnt, o_all_local, o_entry_d) VALUES (?, ?, ?, ?, ?, ?, ?)",
            ...
            for (int d = 1; d <= districtsPerWarehouse; d++) {
                ...
                String districtKey = w_id + "-" + d;
                List<Integer> oids = new ArrayList<>();
                oidsPerDistrict.put(districtKey, oids);
                for (int c = 1; c <= customersPerDistrict; c++) {
                    ...
                    // }
                    // rxy: right now, just set o_carrier_id to null
                    oorder.o_carrier_id = null;
                    ...
                    openOrderStatement.executeUpdate();
                    try (ResultSet rs = openOrderStatement.getGeneratedKeys()) {
                        if (rs.next()) {
                            oids.add(rs.getInt(1));
                        } else {
                            throw new SQLException("Failed to get generated o_id for oorder");
                        }
                    }
                }
            }
    ...
    protected void loadNewOrders(Connection conn, int w_id, int districtsPerWarehouse, int customersPerDistrict) {
        ...
        try (PreparedStatement newOrderStatement = getInsertStatement(conn, TPCCConstants.TABLENAME_NEWORDER)) {
            ...
            for (int d = 1; d <= districtsPerWarehouse; d++) {
                String districtKey = w_id + "-" + d;
                List<Integer> oids = oidsPerDistrict.get(districtKey);
