...
public class StockLevel extends TPCCProcedure {
    ...
    public SQLStmt stockGetOpenOrderIdSQL = new SQLStmt(
            "SELECT MAX(o_id) AS max_o_id, MIN(o_id) AS min_o_id " +
            " FROM " + 
            " (SELECT o_id FROM " + TPCCConstants.TABLENAME_OPENORDER +
            " WHERE O_W_ID = ? " +
            " AND O_D_ID = ? " +
            " ORDER BY O_ID DESC LIMIT 20) AS last_orders");
    ...
    public void run(Connection conn, Random gen, int w_id, int numWarehouses, int terminalDistrictLowerID, int terminalDistrictUpperID, TPCCWorker w) throws SQLException {
        ...
        int[] orderIdRange = new int[2];

        // int o_id = getOrderId(conn, w_id, d_id);
        orderIdRange = getOrderIdRange(conn, w_id, d_id);

        int stock_count = getStockCount(conn, w_id, threshold, d_id, orderIdRange);
    ...
    private int[] getOrderIdRange(Connection conn, int w_id, int d_id) throws SQLException {
        try (PreparedStatement stockGetOpenOrderId = this.getPreparedStatement(conn, stockGetOpenOrderIdSQL)) {
            stockGetOpenOrderId.setInt(1, w_id);
            stockGetOpenOrderId.setInt(2, d_id);

            try (ResultSet rs = stockGetOpenOrderId.executeQuery()) {

                if (!rs.next()) {
                    throw new RuntimeException("O_W_ID=" + w_id + " O_D_ID=" + d_id + " not found!");
                }
                return new int[]{rs.getInt("max_o_id"), rs.getInt("min_o_id")};
            }
        }
    ...
    private int getStockCount(Connection conn, int w_id, int threshold, int d_id, int[] orderIdRange) throws SQLException {
        try (PreparedStatement stockGetCountStock = this.getPreparedStatement(conn, stockGetCountStockSQL)) {
            ...
            stockGetCountStock.setInt(3, orderIdRange[0]);
            stockGetCountStock.setInt(4, orderIdRange[1]);
            stockGetCountStock.setInt(5, w_id);
            stockGetCountStock.setInt(6, threshold);

            try (ResultSet rs = stockGetCountStock.executeQuery()) {
                if (!rs.next()) {
                    String msg = String.format("Failed to get StockLevel result for COUNT query [W_ID=%d, D_ID=%d, O_ID=%d]", w_id, d_id, orderIdRange[0]);
