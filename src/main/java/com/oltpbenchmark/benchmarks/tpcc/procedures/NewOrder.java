...
public class NewOrder extends TPCCProcedure {
    ...
    public final SQLStmt stmtInsertOOrderSQL = new SQLStmt(
            ...
            " (O_W_ID, O_D_ID, O_C_ID, O_OL_CNT, O_ALL_LOCAL, O_ENTRY_D) " +
            " VALUES (?, ?, ?, ?, ?, ?)");
    ...
    public void run(Connection conn, Random gen, int terminalWarehouseID, int numWarehouses, int terminalDistrictLowerID, int terminalDistrictUpperID, TPCCWorker w) throws SQLException {
        ...
        try {
            // First Transaction: Order Header
            int order_id = newOrderTransaction(terminalWarehouseID, districtID, customerID, numItems, allLocal, conn);
            conn.commit();
            
            for (int i = 0; i < numItems; i++) {
                // Second Transaction: add an Order Line
                addItemTransaction(terminalWarehouseID, districtID, order_id, i + 1, allLocal, itemIDs, supplierWarehouseIDs, orderQuantities, conn);
                conn.commit();
            }
        } catch (SQLException e) {
            throw e; // Rethrow unexpected errors to worker
        }
    }


    private int newOrderTransaction(int w_id, int d_id, int c_id, 
                                    int o_ol_cnt, int o_all_local, Connection conn) throws SQLException {
        ...
        int order_id = insertOpenOrder(conn, w_id, d_id, c_id, o_ol_cnt, o_all_local);

        // insertNewOrder(conn, w_id, d_id, d_next_o_id);
        insertNewOrder(conn, w_id, d_id, order_id);

        return order_id;
    
    }

    private void addItemTransaction(int w_id, int d_id, int o_id,
                                     int ol_number, int o_all_local, int[] itemIDs,
        ...
        try (PreparedStatement stmtUpdateStock = this.getPreparedStatement(conn, stmtUpdateStockSQL);
            ...
            // Insert Order Line for this item
    ...
    private int insertOpenOrder(Connection conn, int w_id, int d_id, int c_id, int o_ol_cnt, int o_all_local) throws SQLException {
        try (PreparedStatement stmtInsertOOrder = this.getPreparedStatementReturnKeys(conn, stmtInsertOOrderSQL, new int[]{1});) {
            // Request generated keys from the DB
            stmtInsertOOrder.setInt(1, w_id);
            stmtInsertOOrder.setInt(2, d_id);
            stmtInsertOOrder.setInt(3, c_id);
            stmtInsertOOrder.setInt(4, o_ol_cnt);
            stmtInsertOOrder.setInt(5, o_all_local);
            stmtInsertOOrder.setTimestamp(6, new Timestamp(System.currentTimeMillis()));
            stmtInsertOOrder.executeUpdate();
            try (ResultSet generatedKeys = stmtInsertOOrder.getGeneratedKeys()) {
    ...
    private void getDistrict(Connection conn, int w_id, int d_id) throws SQLException {
