...
import com.oltpbenchmark.benchmarks.spreenorpc.procedures.NewOrder;
...
public class SpreenorpcBenchmark extends BenchmarkModule {
    private static final Logger LOG = LoggerFactory.getLogger(SpreenorpcBenchmark.class);

    public SpreenorpcBenchmark(WorkloadConfiguration workConf) {
        super(workConf);
    }
    ...
    @Override
    protected Loader<SpreenorpcBenchmark> makeLoaderImpl() {
    ...
    protected List<SpreeWorker> createTerminals() throws SQLException {
        ...
        for (int w = 0; w < numWarehouses; w++) {
            ...
            for (int terminalId = 0; terminalId < numWarehouseTerminals; terminalId++) {
                ...
                SpreeWorker terminal = new SpreeWorker(this, workerId++, w_id, lowerDistrictId, upperDistrictId, numWarehouses, mergeSize);
