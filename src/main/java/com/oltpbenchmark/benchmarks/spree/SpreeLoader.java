...
import com.oltpbenchmark.benchmarks.spree.pojo.*;
 ...
 * Spree Loader
 */
public class SpreeLoader extends Loader<SpreeBenchmark> {

    private final long numWarehouses;

    private final Random rand = new Random();

    public SpreeLoader(SpreeBenchmark benchmark) {
        super(benchmark);
    ...
    protected void loadOpenOrders(Connection conn, int total_users) {
        ...
        try (PreparedStatement openOrderStatement = getInsertStatement(conn, SpreeConstants.TABLENAME_SPREEORDERS)) {
            ...
            for (int c = 1; c <= total_users; c++) {
                ...
                oorder.store_id = 1;
                String num = "R" + SpreeUtil.randomNStr(30);
                oorder.number = num;
